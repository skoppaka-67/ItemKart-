...
def login():
    ...
    if request.method == 'POST':
        ...
        if request.form['username'] == User_DATA['name'] and request.form['password'] == User_DATA['password']:
            ...
            token = jwt.encode({'username': 'John Doe', 'exp': datetime.datetime.utcnow() + datetime.timedelta(minutes=5)},
...
def category():
    ...
    cursy = db.items.find({'_id':{'$gt':last_id},"tags": category},{"name":1,"price":1,"description" : 1}).sort('_id',pymongo.ASCENDING).limit(limit)
    # datavalue[item_tag] =[record for record in cursy]
    for rec in  cursy:
        temp_json[category] = rec
        data.append(copy.deepcopy(temp_json))
        temp_json.clear()
    next_url  = '/category'+ '?category_list='+ str(category)+'&limit=' + str(limit) + '&offset=' + str(offset+limit)
    prev_url  = '/category'+ '?category_list='+str(category) +'&limit=' + str(limit) + '&offset=' + str(offset-limit)
